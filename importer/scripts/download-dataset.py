...
def extract_filtered_links(site, prefix):
    html = requests.get(site).text
    soup = BeautifulSoup(html, 'html.parser').find_all('a')
    # Filtrer les liens pour ne garder que ceux dont l'attribut href commence par le préfixe donné
    links = [link.get('href') for link in soup if link.get('href', '').startswith(prefix)]
    return links
...
def download_url(url):
    ...
    try:
        ...
        print(filename + " téléchargement échoué")
        print(err)
        sys.exit(0)
...
if __name__ == "__main__":
    ...
    prefix = "PARCELLAIRE_EXPRESS_1-1"  # Préfixe à utiliser pour le filtrage
    ...
    all_links = extract_filtered_links(idx_url, prefix)  # Utilise la fonction modifiée
